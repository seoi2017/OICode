...
inline int Gauss(){
    ...
    for(int j=1;i<=n,j<=n;i++,j++){
        int index=i;
        for(int k=i+1;k<=n;k++)
            if(fabs(mt[k][j])-fabs(mt[index][j])>eps)index=k;
...
void dfs(int x,int l,int r,double p){//DFS构造方程
    int sons=0,ks=0;
    bool noks=true;
    for(int i=l;i<=r;i++){
        if(str[i]==' ')continue;
    ...
    if(noks){
        for(int i=l;i<=r;i++){
            if(str[i]>='a' && str[i]<='z')
                ...
                int tmp;
                sscanf(str+i,"%d",&tmp);//直接从字符串中读入数据，免去处理负数的烦恼
                mt[x][n+1]+=p*tmp;
                i++;
                while(str[i]!=' ' && i<=r)i++;
            }
...
int main(){
    ...
    for(int i=1;i<=n;i++){
        printf("%c ",'a'+i-1);
        if(ans<0 ||(ans>0 && !unline[i]))printf("= Unknown\n");
        else printf("= %.3lf\n",x[i]);
    }
    fclose(stdin);
    fclose(stdout);
